...
async def fetch_text(
    ...
    notebooks. Using the default `pandas.read_csv` for example does not support
    `https://` URLs from JupyterLite (Pyodide).
    ...
    async with executor.execute(
        PlainTextExecutorResult, url, headers=headers
    ) as res:
